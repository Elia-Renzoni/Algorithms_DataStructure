...
int main(void) {
    ...
    printf("Indirizzo > : %d", indirizzo_oggetto_lista);
...
caratteri_t *creazione_lista(int *numero_liste) {
    ...
    if (valore_inserire != SENTINELLA) {
        ...
        for (scanf("%c", &valore_inserire); (valore_inserire != SENTINELLA); scanf("%c", &valore_inserire)) {
            ++(*numero_liste);
            oggetto_lista = oggetto_lista->successivo = (caratteri_t *)malloc(sizeof(caratteri_t));
            oggetto_lista->valori = valore_inserire;
        }
...
caratteri_t *ricerca_sequenziale(caratteri_t *testa_lista, int *numero_liste) {
    ...
    double       media = *numero_liste / 2;
    int          contatore = 0;

    if (media > 2) {
        for (oggetto_lista = testa_lista, contatore = 1; (contatore != media); oggetto_lista = oggetto_lista->successivo, contatore++); 
